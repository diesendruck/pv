""" 
    ...
    This script adds noise to data and measures the effect on moments.
    NOTHING GENERATIVE HAPPENS HERE. Just descriptive measures on samples.
    See line ~1108.
...
from mmd_utils import compute_mmd, compute_kmmd, compute_cmd, compute_moments, MMD_vs_Normal_by_filter, privacy_risk
    ...
    if data_file:
        ...
        if data_dimension == 1:
            ...
            for i in range(data_num):
                ...
                cluster_i = np.random.binomial(1, 0.7)  # NOTE: Setting p=0/p=1 chooses one cluster.
                if cluster_i == 0:
...
def main():
    ...
    lambda_init = 0.01
    lambda_step = 0.1
    all_data = np.zeros((num_runs, data.shape[0], data.shape[1]))
    all_loss_priv = np.zeros((num_runs, 1))
    all_loss_acc = np.zeros((num_runs, 1))

    # 0. Measure initial privacy risk.
    base_risk, _ = privacy_risk(data)
    # 1. Initialize lambda with small value.
    lambda_i = lambda_init
    data_i = data
    for i in range(num_runs):
        # 2. Perturb data.
        data_noised_i = data_i + np.random.laplace(
            size=data_i.shape, loc=0, scale=lambda_i)
        # 3. Compute new losses.
        loss_type = 'kl'
        loss_priv_i, _ = privacy_risk(data_noised_i, loss_type=loss_type)
        loss_acc_i = compute_mmd(data, data_noised_i, slim_output=True)
        
        # Store states and increment vars. 
        all_data[i] = data_noised_i
        all_loss_priv[i] = loss_priv_i
        all_loss_acc[i] = loss_acc_i 
        data_i = data_noised_i
        lambda_i += lambda_step

    # Plot the errors all together.
    fig, ax = plt.subplots(4, 1)
    cmap = plt.cm.get_cmap('cool', k_moments)
    ax[0].plot(all_loss_priv, label='loss_priv')
    ax[1].plot(all_loss_acc, label='loss_acc')
    ax[0].set_xlabel('run, lambda val')
    ax[0].set_ylabel('l_priv')
    ax[1].set_ylabel('l_acc')

    ax[2].hist(data, normed=True, bins=10, alpha=0.5)
    ax[2].set_xlabel('data')
    ax[3].hist(data_noised_i, normed=True, bins=10, alpha=0.5)
    ax[3].set_xlabel('data_noised, lambda={}'.format(lambda_i))
    #plt.suptitle('privacy risk')
    plt.tight_layout()
    plt.savefig(os.path.join(
        plot_dir, 'result_privacy_risk.png'))
    plt.close(fig)


    sys.exit('--- PRIVACY RISK. END. ---')
    ...
    plt.suptitle('data vs dp({})_data, L1 error of moments'.format(noise_type))
    plt.tight_layout()
    plt.savefig(os.path.join(
        plot_dir, 'result_moment_effect_{}.png'.format(noise_type)))
    plt.close(fig)
