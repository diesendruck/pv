...
def compute_cmd(arr1, arr2, k_moments=2, use_tf=False, cmd_span_const=None,
        return_terms=False, coordinates=None, taylor_weight=False):
    ...
    if use_tf:
        ...
        first_term = cmd_span_const * tf.norm(arr1_mean - arr2_mean, ord=2)
        cmd += first_term
...
def compute_noncentral_moment_discrepancy(
    ...
    if use_tf:
        ...
        for k in range(2, k_moments + 1):
            ...
            term_k = (cmd_span_const**k) * tf.norm(arr1_mk - arr2_mk)
            cmd += term_k
            terms.append(term_k)
        ...
        for k in range(2, k_moments + 1):
            ...
            term_k = (cmd_span_const**k) * np.linalg.norm(arr1_mk - arr2_mk)
            cmd += term_k
            terms.append(term_k)
...
def compute_joint_moment_discrepancy(
    ...
    if use_tf:
        ...
        for k in [1]:
            ...
            term_k = (cmd_span_const**k) * tf.norm(arr1_jmk - arr2_jmk)
            cmd += term_k
            terms.append(term_k)
        ...
        for k in [1]:
            ...
            # Add k'th term to collection.
            term_k = (cmd_span_const**k) * np.linalg.norm(arr1_jmk - arr2_jmk)
            cmd += term_k
            terms.append(term_k)
